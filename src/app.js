...
app.use('/api', async function(req, res, next) {
    const accessToken = req.query['access_token'] || req.header('access_token') || req.body['access_token'];
    ...
    try {
        const response = await axios.get(`https://gitlab.empressia.pl/api/v4/user?access_token=${accessToken}`)
        const user = response.data

        if (!user.is_admin || user.state !== 'active') {
            return next(createError(403, 'Access Forbidden. You must be Gitlab\'s administrator to access API.'))
        }
